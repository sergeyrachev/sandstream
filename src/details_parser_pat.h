...
namespace details{
    ...
    std::tuple<size_t, challenge::pat_t> parse_pat_payload( size_t payload_size, const Storage &storage, size_t position){
        using masked_two_bytes_value_t = challenge::masked_two_bytes_value_t<Storage>;
        ...
        return {payload_position + position, services};
    }
}
